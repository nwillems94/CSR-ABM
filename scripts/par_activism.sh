...
function concat_runs {
    if [ $1 == "log" ]; then
        awk '(NR == 1) || (FNR > 1)' `ls -v ./logs/param_log_$2-{1..$nruns}.csv` > ./logs/param_log_$2.csv || break 
        rm ./logs/param_log_$2-{1..$nruns}.csv 
        eval "cat ./logs/run_log_$2-{1..$nruns}.txt >> ./logs/run_log_$2.txt"
        eval "rm  ./logs/run_log_$2-{1..$nruns}.txt"
    else
        awk '(NR == 1) || (FNR > 1)' `ls -v ./outputs/$1_states_$2-{1..$nruns}.csv` > ./outputs/$1_states_$2.csv || break 
        rm ./outputs/$1_states_$2-{1..$nruns}.csv 
    fi
}
