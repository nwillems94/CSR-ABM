...
dist_social_pressure <- function(dt_f, ti, focus=1) {
    ...
    if (ti$strategy == "even") {
        ...
        dt_f[
            (firmID %in% focus) & (behavior == "flaring"),
            "sPressure":= a / .N
        ]
    } else if (ti$strategy == "gas_output") {
        dt_f[
            behavior == "flaring",
            "sPressure":= a * gas_output / sum(gas_output)
        ]
...
lease_costs <- function(dt_l) {
    ...
    # MCF to BOE Conversion:
    #    https://petrowiki.spe.org/Glossary:Barrels_of_oil_equivalent
    dt_l[,
        {# amount of casinghead gas produced
        ...
        opEx_pBOE_mod= opEx_pBOE * BOE_emp /
            (BOE_base + csgd_developed_MCF / 6);
        # capital expenditures paid off over lease lifetime
        capEx_pBOE= capEx / lifetime / BOE_base;
        capEx_csgd_pMCF=
            nafill(capEx_csgd /
                (lifetime + t_found - t_switch) /
                    csgd_developed_MCF, fill=0);
        #   production
        .(cbind("oil"=oil_BBL + cond_BBL,
                "csgd"=csgd_developed_MCF,
                "gas"=gas_MCF) *
...
calc_debits <- function(dt_f, dt_l) {
    ...
    dt_f[
        dt_l[,
            .SD[
                (is.na(market) | market != "none") & (status == "producing"),
                sum(cost_oil + cost_gas)
                ],
            by=firmID
        ], on="firmID",
        "cost_P":= cost_P + V1
...
calc_credits <- function(dt_f, market) {
    dt_f[,
        "gas_revenue":= (grey_gas_sold * market$p_grey +
                        green_gas_sold * market$p_green)
    ]
    dt_f[,
        "oil_revenue":= (oil_output * (market$p_grey * market$p_oil_mult))
    ]
...
imitators <- function(dt_f, dt_p, ti, success_metric="sales") {
    ...
    #    less successful followers mimic more successful leaders
    # following Leary & Roberts "success" can be defined in terms of
    #    sales (market share), profit, or market_value
    observations <- dt_f[,
                        {"weight"= 1 + get(success_metric);
                        # agents doing exploration cannot imitate
                        "I"= which(activity == "development");
                        .("observer"= firmID[I],
                        "weight"= weight[I],
                        "imitation"= NA_character_,
                        # more successful agents are more likely to be imitated
                        "observed"= sapply(I, function(x)
                            sample(behavior[-x], 1, prob=weight[-x])))
                        }
                    ]

    ## agents considering staying grey may instead copy other green firms
    observations[
        dt_p[(option == "grey") & !is.na(K)], on=c(observer="firmID"),
        "imitation":= replace(imitation,
            observed %in% c("economizing", "mitigating", "imitating"), "green")
    ]
    ## agents considering going green may change its mind
    #    (as long as it's not happening because of standard development)
    if (!((ti$strategy == "shock") & between(ti$time, 1, 12))) {
        observations[
            dt_p[(option == "green") & (K > 0)], on=c(observer="firmID"),
    ...
    return(observations[
        sample(.N, sum(runif(.N) < ti$prob_m), prob=1 / weight)][
        !is.na(imitation)]$observer)
...
optimal_strategy <- function(dt_f, dt_l, dt_m, demand_schedule, ti) {
    ...
    firmIDs <- union(
        dt_l[(status == "pending") & (class == "underdeveloped"), firmID],
    ...
    dt_p <- dt_l[order(AC_pMCF)][firmID %in% firmIDs][
                (lifetime + t_found - 1) > ti$time,
    ...
    dt_p[activity == "exploration",
        "leaseIDs":= .(.(dt_l[
            (firmID == .BY) & (status == "pending") &
                (class == "underdeveloped"),
            leaseID
        ])),
        by=firmID
    ]

    # develop leases which would have an average cost
    #    below the conventional market price for gas
    dt_p[,
        "base_perm":= cbind(dt_l, AC_pMCF)[
            leaseIDs,
            .(.(as.numeric(AC_pMCF < p_grey)))
            ],
        by=firmID
    ]

    # for firms doing development, calculate additional gas capture
    #    (wrt base) necessary to meet green threshold
    dt_p[,
        "K":= sopf_MCF +
            dt_l[leaseIDs, (1 - base_perm[[1]]) %*% csgd_MCF] -
            (ti$threshold * prod_BBL),
        by=firmID
    ]
    ...
    dt_p[
        lengths(green_perm) < lengths(base_perm),
        "green_perm":=
            .(ifelse(K <= 0 | is.na(K), base_perm,
                Map(pmax, base_perm,
                        dt_l[leaseIDs, shift(cumsum(csgd_MCF) < K, fill=1)]))),
        by=firmID
    ]

    # calculate the additional cost and revenue
    #    associated with exercising the green option
    #    no need to calculate where base and green permutations are the same
    dt_p[
        sapply(Map(`==`, base_perm, green_perm), all),
        c("cost_M_add", "gas_revenue_add"):= 0
    ]
    ...
    if (dt_p[is.na(gas_revenue_add), .N] > 0) {
        ...
        if (ti$market_prop_green == 0) {
            dt_p[
                is.na(gas_revenue_add),
                "green_revenue":=
                    dt_l[
                        (firmID == .BY) & ((lifetime + t_found - 1) > ti$time),
                        # account for existing and green leases
                        (nafill(green_perm[[1]][
                                match(leaseID, leaseIDs[[1]])],
                            fill=1) * ERR_MCF) %*%
            ...
            dt_p[
                ...
                "green_revenue":=
                    dt_l[
                        ...
                        # account for existing and green leases
                        (nafill(green_perm[[1]][
                                match(leaseID, leaseIDs[[1]])], fill=1)
                            * ERR_MCF) %*%
                        # account for where gas is sold
                        (ifelse((opEx_pMCF > p_grey) & (opEx_pMCF <= p_green),
                                p_green,
        ...
        dt_p[
            is.na(gas_revenue_add),
            "gas_revenue_add":= green_revenue - base_revenue
        ]
    }
    ...
    dt_p[
        dt_f, on="firmID",
        "sPressure":= sPressure *
            max(dt_l[firmID == .BY, lifetime + t_found - ti$time - 1]),
        by=.EACHI
    ]
    ...
    dt_p[,
        "economical":= ((K <= 0) | (gas_revenue_add > cost_M_add))
    ]
    dt_p[,
        "option":= fifelse(economical == TRUE,
            "green", NA_character_, na=NA_character_)
    ]
    dt_p[
        activity == "exploration",
        "option":= replace(option, is.na(option), "grey")
    ]

    # with activist pressure?
    # if the possible threat outweighs the cost, exercise the mitigation option
    dt_p[
        is.na(option),
        "option":= ifelse((cost_M_add * (1 - ti$SRoR)) -
                gas_revenue_add < sPressure,
            "green", "grey")
    ]

    # imitators will ignore economics and copy a the behavior of a successful firm
    dt_p[,
        "imitation":= (firmID %in% imitators(dt_f, dt_p, ti))
    ]
    # reverse decision because of imitation
    dt_p[
        imitation == TRUE,
        "option":= unname(c("grey"="green", "green"="grey")[option])
    ]
    # only register grey-to-green as imitators
    dt_p[
        (imitation == TRUE) & (option == "grey"),
        "imitation":= FALSE
    ]


    # optimal decision for which assets to develop
    dt_p[
        !is.na(option),
        "devIDs":= Map(`[`, leaseIDs,
            lapply(ifelse(option == "green", green_perm, base_perm),
                as.logical))
    ]
...
do_development <- function(dt_f, dt_l, dt_p, ti) {
    ...
    dt_l[
        .(dt_p[, unlist(devIDs)]),
        sprintf("cost_%s", c("oil", "csgd", "gas")):= lease_costs(.SD)
    ]

    ## Update firm attributes
    # whether they are mitigating and if they are
    #    doing so because of simple economics (besides social pressure)
    dt_f[
        dt_p, on="firmID",
        "behavior":= ifelse(option == "green", "mitigating", "flaring")
    ]
    dt_f[
        dt_p[economical == TRUE], on="firmID",
        "behavior":= "economizing"
    ]
    dt_f[
        dt_p[imitation == TRUE], on="firmID",
        "behavior":= "imitating"
    ]
...
do_exploration <- function(dt_f, dt_l, ti) {
    ...
    # new discovered leases begin as undeveloped,
    #    then progress to underdeveloped with pending production
    #   (at this point, firms can decide whether to fully develop them),
    #    then finally begin production
    dt_l[status == "pending", "status":= "producing"]
    dt_l[
        class == "undeveloped",
        c("class", "status"):=
            .(ifelse(csgd_MCF == 0, "developed", "underdeveloped"), "pending")
    ]
    # retire leases at the end of their lifetime
    dt_l[.(retiring_leases), "status":= "retired"]

    N_discs <- dt_l[, dcast(.SD, firmID~OIL_GAS_CODE,
                fun=function(x) sum(x != "retired"), value.var="status")][
                    dt_f[activity == "exploration"], on="firmID"][,
                    sum(runif(.N) < ti$prob_e *
                    dt_l[
                        .(retiring_leases),
                        sum(oil_BBL + cond_BBL + (gas_MCF + csgd_MCF) / 6)
                    ])
                - sum(((O == 0) & (production_BBL > 0)) +
                    ((G == 0) & (production_MCF > 0)))
                ]

    new_discs <- dt_l[status == "retired"][
                    dt_f[activity == "exploration"], on="firmID"][,
                    -sample(-firmID, max(ceiling(N_discs), 0),
                        prob=1 + (leaseID %in% retiring_leases))
                ]

    # agents with no assets will discover new ones
    new_discs <- dt_l[, dcast(.SD, firmID~OIL_GAS_CODE,
                    fun=function(x) sum(x != "retired"), value.var="status")][
                        dt_f[activity == "exploration"], on="firmID",
                        c(new_discs,
                            setdiff(firmID[(O == 0) & (production_BBL > 0)],
                                new_discs),
                            setdiff(firmID[(G == 0) & (production_MCF > 0)],
                                new_discs))
                    ]
    ...
    if (length(new_discs) > 0) {
        ...
        weights <- dt_l[status == "retired"][
                    .(new_discs), on="firmID", allow.cartesian=TRUE, nomatch=0,
                    sum(1 + (leaseID %in% retiring_leases)),
                    keyby=.(OIL_GAS_CODE)
                    ][
                        .(c("O", "G")),
                        setNames(nafill(V1, fill=0), OIL_GAS_CODE)
                ]
        ...
        weights <- dt_l[
                    ...
                    by=OIL_GAS_CODE
                        ...
                        weights[OIL_GAS_CODE] / N
                ]

        # assign newly discovered leases to firms
        new_leases <- dt_l[is.na(firmID)][
                        sample(.N, length(new_discs),
                            prob=weights[OIL_GAS_CODE])
                    ]

        dt_l[
            .(new_leases$leaseID),
            c("class", "t_found"):= .("undeveloped", ti$time)
        ]
        dt_l[.(new_leases[OIL_GAS_CODE == "G"][
            order(-capEx), leaseID]),
            "firmID":= dt_f[
                            .(new_discs)][,
                            "shift":= ifelse(production_MCF > 0,
                                exp(mean(log(1 + production_MCF))), 0)
                            ][,
                                -sample(-firmID, length(leaseID),
                                prob=1 + production_MCF + shift)
                        ]
        ]
        dt_l[.(new_leases[OIL_GAS_CODE == "O"][
            order(-capEx), leaseID]),
            "firmID":= dt_f[
                            .(new_discs)][,
                            "shift":= ifelse(production_BBL > 0,
                                exp(mean(log(1 + production_BBL))), 0)
                            ][,
                                -sample(-firmID, length(leaseID),
                                    prob=1 + production_BBL + shift)
                        ]
        ]
...
market_allocation <- function(dt_l, demand_schedule, firmIDs, mp_grey) {
    ...
    if (b_green < mp_grey) {
        ...
        # upper bound on q_shift
        #    by assuming supply curve is vertical at intersection with demand
        q_shift <- -q_green * (1 -
                    (mp_grey - b_green) /
                        (demand_schedule(q_green / 100)$p_green - b_green) / 100)
    }

    max_profit <- dt_l[firmID %in% firmIDs][
                    order(opEx_pMCF),
                    .SD[
                        !is.na(market) & (opEx_pMCF <= mp_grey)][,
                        .("leaseIDs"= c(NA_integer_, leaseID),
                            "cutoff"= c(0, opEx_pMCF))],
                    by=firmID
                ]

    max_profit[, "sequence":= seq(.N), by=firmID]
    if (q_shift < 0) {
        # green price lower than grey price,
        #    need to remove q_shift or more units from green to grey
        max_profit[
            dt_l[market == "green"], on=c(leaseIDs="leaseID"),
            "MCF":= gas_MCF + csgd_MCF
        ]
        max_profit[,
            "sequence":= replace(sequence,
                cumsum(nafill(MCF, fill=0)) < -q_shift, NA),
            by=firmID
        ]
    } else if (q_shift > 0) {
        # green price higher than grey price,
        #    can add at most q_shift units from grey to green
        max_profit[
            dt_l[market == "grey"], on=c(leaseIDs="leaseID"),
            "MCF":= gas_MCF + csgd_MCF
        ]
        max_profit[,
            "sequence":= replace(sequence,
                rev(cumsum(rev(nafill(MCF, fill=0)))) > q_shift, NA),
            by=firmID
        ]
    }
    ...
    max_profit[,
        "cp_green":= if (.N == 1) market_price(dt_l, demand_schedule, "green")
            else NA_real_,
        by=firmID]
    ...
    max_profit[
        ...
        "cp_green":= dt_l[,
            replace(.SD, "market",
                ifelse((firmID == .BY) & !is.na(market), "green", market))[,
                sapply(sequence, function(x)
                if (is.na(x)) NA else
                    market_price(.SD[!.(leaseIDs[1:x])], demand_schedule, "green"))]],
        by=firmID
    ]

    max_profit[,
        "cp_green":= replace(cp_green,
            (cp_green <= mp_grey) & (sequence < .N), NA),
        by=firmID
    ]
...
clear_gas_markets <- function(dt_f, dt_l, dt_m, demand_schedule, ti) {
    ...
    dt_l[,
        "market":= ifelse((class == "developed") & (status == "producing")
                        & (gas_MCF > 0 | csgd_MCF > 0),
            replace(market, is.na(market) | market == "none", "grey"), NA)
    ]
    dt_l[
        dt_f[behavior == "flaring"], on="firmID",
        "market":= replace(market, !is.na(market), "grey")
    ]
    ...
    if (ti$market_prop_green > 0) {
        ...
        # developing firms determine how to allocate production
        #     between green and grey markets
        optimal_allocation <- dt_f[
                (behavior != "flaring") & (activity == "development"),
        ...
        dt_l[
            optimal_allocation[error < 0], on="firmID",
            "market":= replace(market,
                which.max(rank(-opEx_pMCF) * (market == "green")), "grey"),
            by=.EACHI
        ]
        ...
        dt_l[
            optimal_allocation[error > 0], on="firmID",
            "market":= replace(market,
                which.max(rank(opEx_pMCF) * (market != "green")), "green"),
            by=.EACHI
        ]
    }
    ...
    dt_l[
        market %in% c("green", "grey"),
        "ERR_MCF":= ERR_MCF - (gas_MCF + csgd_MCF)
    ]
    ...
    dt_f[
        dt_l[,
            .SD[market == "grey", sum(gas_MCF + csgd_MCF)], by=firmID
        ], on="firmID",
        "grey_gas_sold":= V1
    ]
    dt_f[
        dt_l[,
            .SD[market == "green", sum(gas_MCF + csgd_MCF)], by=firmID
        ], on="firmID",
        "green_gas_sold":= V1
    ]
...
demand_sample <- function(prop_green, sample_set, p_low, p_high) {
    ...
    cat("Generating demand function from:",
        paste(sample_set[[2]], sample_set[[1]], collapse=", "), "\n")
...
recover_outputs <- function(refID) {
    ...
    data_sql <- "SELECT * FROM %2$s 
                LEFT JOIN %1$s ON %1$s.firmID=%2$s.firmID 
                    ...
                    WHERE %2$s.model=1 
                        AND %2$s.time IN (%3$d, %3$d-1,
                         (SELECT MIN(t0) FROM params WHERE model=1)-1)"
    ...
    market_history <- DBI::dbGetQuery(db,
        "SELECT * FROM market_states WHERE model=1")
...
reset_db <- function(refID) {
    ...
    for (i in c("params", "agent_states", "agent_info", "lease_info", "lease_states", "market_states")) {
        ...
        DBI::dbExecute(db,
            sprintf("DELETE FROM %s WHERE model=1;", i))
        DBI::dbExecute(db,
            sprintf("UPDATE %s SET model=model-1 WHERE model>1;", i))
