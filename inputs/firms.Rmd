...
### (from EIA Drilling productivity report)  
...
lease_prod <- OG_COUNTY_CYCLE[CYCLE_YEAR>=2010 & CYCLE_YEAR<2021,
                    ...
                    # by=.("Date"= as.Date(paste("01", CYCLE_MONTH, CYCLE_YEAR), format='%d %m %Y'))]
# lease_prod[, c("year","month"):= .(year(Date), month.abb[month(Date)])]
...
print(summary(lease_prod[, (gas_subset+csgd_subset)/(gas_total+csgd_total)]))
```
