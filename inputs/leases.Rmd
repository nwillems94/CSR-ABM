...
EACH LEASE HAS THE FOLLOWING EMPRICAL PROPERTIES  
...
wellbore_data[start<1000, "start":= NA_integer_]
```
...
## Permit (contains well type & depth)
...
```{r}
OG_WELL[!duplicated(API_NO), sum(shared, na.rm=TRUE) / .N, by=.("year"=as.numeric(substr(start, 1, 4)))][,
...
print(summary(OG_LEASE_CYCLE_DISP[gas_MCF > 0, cond_BBL / (gas_MCF/6)]))
...
print(OG_LEASE_CYCLE_DISP[OIL_GAS_CODE=="G", as.list(summary(flared_MCF/gas_MCF)),
...
OG_LEASE_AVG <- OG_LEASE_CYCLE_DISP[OG_IDS[is.na(drop) | drop==FALSE],
                                    ...
                                    .SD[, sum(total_BOE), keyby=CYCLE_YEAR_MONTH][
...
# Estimate the total recoverable gas using EIA decline curves
