...
EACH LEASE HAS THE FOLLOWING PROPERTIES  
  ...
  Cost data is derived by matching leases to areas and sampling data from: [US EIA. (2016)](zotero://select/items/0_EJYISQT4) 
  and is assigned during initialization
...
from: http://webapps2.rrc.texas.gov/EWA/help/prod_results_questions.html:  
...
```{r}
# use disposition instead of production (e.g., LEASE_OIL_PROD_VOL)
#   this maintains consistency with flaring volumes and volumes move by pipeline, rail, or truck
...
print(summary(OG_LEASE_CYCLE_DISP[OIL_GAS_CODE=="O", sum(gas_MCF+csgd_MCF), by=.(DISTRICT_NO, LEASE_NO)]$V1))
...
print(OG_LEASE_CYCLE_DISP[OIL_GAS_CODE=="G", sum(csgd_MCF)])
...
print(OG_LEASE_CYCLE_DISP[OIL_GAS_CODE=="G", as.list(summary(gas_flared_MCF/(gas_MCF+csgd_MCF))),
...
# Wellbore Data   
Get approximate locations & depths of leases based on member wells  
...
Extract relevant wellbore data
```{r}
wellbore_data <- wellbore_full[sort(c(unique_bors, unique_locs)),
                                # API number & depth from record 01
...
```{r}
OG_WELL[wellbore_data, on="API_NO", c("lat","lon","depth"):= .(lat, lon, depth)]
...
plot(areas$geometry, col=rainbow(4, alpha=0.5), reset=FALSE, main="Gas plants and crude oil pipelines in Texas")
...
spatial_data <- list("areas"= areas,
                    "gas_plants"= gas_plants[gas_plants$State %in% c("LA","NM","OK","TX"),],
                    "crude_pipelines"= crude_pipelines)

rm(permian, spraberry, eagle_ford, areas, gas_plants, crude_pipelines)
```
...
# assume the last lease entry is the most current
OG_WELL[API_NO %in% API_NO[duplicated(API_NO)], "area":= last(area), by=API_NO]
OG_WELL[duplicated(API_NO, fromLast=TRUE), "area":= NA]
OG_WELL[is.na(area), "depth":= NA]

plot(spatial_data$areas$geometry, col=rainbow(4, alpha=0.3), main="Well locations", reset=FALSE)
...
## Merge well depths and areas with lease disposition data
```{r}
OG_LEASE_AVG[
            # aggregate by district & lease
            OG_WELL[!is.na(area) & !is.na(depth), .(mean(lat), mean(lon), .N), by=.(DISTRICT_NO, LEASE_NO, area)],
            on=c("DISTRICT_NO", "LEASE_NO"), c("area","lat", "lon", "N"):= .(area, V1, V2, N)]
cat("Matched", OG_LEASE_AVG[!is.na(lat), .N], "leases with wells & locations in Eagle Ford & Permian basins")

plot(spatial_data$areas$geometry, col=rainbow(4, alpha=0.3), main="Approximate lease locations", reset=FALSE)
legend("bottomleft", spatial_data$areas$Name, fill=rainbow(4, alpha=0.3))
