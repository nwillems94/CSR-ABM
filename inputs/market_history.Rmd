...
[Natural gas delivered to consumers](https://www.eia.gov/dnav/ng/ng_cons_sum_dcu_nus_m.htm)
```{r}
...
monthly_quantity <- fread("./data/Natural_Gas_Delivered_to_Consumers_in_the_U.S..csv")
monthly_quantity[, c("Date","Month"):= .(as.Date(paste0("01", Month), "%d %b %Y"), NULL)]
```
...
setnames(demand_data, "Natural Gas Delivered to Consumers in the U.S.  Million Cubic Feet", "q")
...
# price outliers
demand_data[, "outlier":=
                    {resid = residuals(loess(p~seq(.N)));
                    resid.q = quantile(resid, probs = c(0.25, 0.75), na.rm = TRUE);
                    iqr = diff(resid.q);
                    limits = resid.q + (iqr * c(-2, 2));
                    (resid < limits[1]) | (resid > limits[2])}]

# quantity outliers
demand_data[, "outlier":=
                    {resid = residuals(loess(q~seq(.N)));
                    resid.q = quantile(resid, probs = c(0.25, 0.75), na.rm = TRUE);
                    iqr = diff(resid.q);
                    limits = resid.q + (iqr * c(-2, 2));
                    (resid < limits[1]) | (resid > limits[2]) | outlier}]
...
ggplot(demand_data, aes(x=q, y=p)) +
    ...
    labs(title="Natural gas prices and quantities delivered", x="quantity", y="price")
```
...
ggplot(demand_data, aes(x=q, y=p)) +
    ...
    geom_point(data=demand_data[outlier==TRUE], color="red") +
    facet_wrap(~season) +
    labs(title="Seasonal natural gas prices and quantities delivered", x="quantity", y="price")
```
...
ggplot(demand_data, aes(x=q, y=p)) +
    ...
    facet_wrap(~month) +
    labs(title="Monthly natural gas prices and quantities delivered", x="quantity (10^6)", y="price")
```

### Linear fit to cleaned monthly data
```{r}
fit_data <- demand_data[outlier==FALSE, .(p, q), by=month]
...
ggplot(fit_data, aes(x=q, y=p)) +
    ...
    facet_wrap(.~month) +
    labs(title="Monthly natural gas demand functions", x="quantity (10^6)", y="price")
```

### Proportion of national demand met by Texas supply
[Texas natural gas marketed production](https://www.eia.gov/dnav/ng/hist/n9050tx2m.htm)
```{r}
supply      <- fread("./data/U.S._Natural_Gas_Marketed_Production.csv")
supply[, "Date":= as.Date(paste0("01", Month), "%d %b %Y")]
supply_TX   <- fread("./data/Texas_Natural_Gas_Marketed_Production.csv")
supply_TX[, "Date":= as.Date(paste0("01", Month), "%d %b %Y")]

demand_data[supply, on="Date", "s":= 1000 * `U.S. Natural Gas Marketed Production  Million Cubic Feet`]
demand_data[supply_TX, on="Date", "s_TX":= 1000 * `Texas Natural Gas Marketed Production  Million Cubic Feet`]
```

```{r fig.cap="Texas production meets about ~1/3 of national gas demand"}
ggplot(demand_data) +
    geom_boxplot(aes(x=year, y=s_TX/q, group=year)) +
    geom_hline(aes(yintercept=mean(s_TX/q)), color="blue") +
    geom_text(aes(x=2002, y=mean(s_TX/q)*0.985,
                label=sprintf("Mean: %.4f", mean(s_TX/q)))) +
    geom_hline(aes(yintercept=median(s_TX/q)), color="blue", lty="dashed") +
    geom_text(aes(x=2003, y=median(s_TX/q)*1.015,
                label=sprintf("Median: %.4f", median(s_TX/q)))) +
    geom_hline(yintercept=demand_data[year>2010, mean(s_TX/q)], color="red") +
    geom_text(data=demand_data[year>2010], aes(x=2010, y=mean(s_TX/q)*1.015,
                label=sprintf("Mean>2010: %.4f", mean(s_TX/q)))) +
    geom_hline(yintercept=demand_data[year>2010, median(s_TX/q)], color="red", lty="dashed") +
    geom_text(data=demand_data[year>2010], aes(x=2011, y=median(s_TX/q)*1.015,
                label=sprintf("Median>2010: %.4f", median(s_TX/q)))) +
    labs(title="Ratio of Texas production to national consumption", y="Texas supply as % of demand")
```

```{r fig.cap="Texas production accounts for about ~1/4 of national gas supply"}
ggplot(demand_data) +
    geom_boxplot(aes(x=year, y=s_TX/s, group=year)) +
    geom_hline(aes(yintercept=mean(s_TX/s)), color="blue") +
    geom_text(aes(x=2002, y=mean(s_TX/s)*1.01,
                label=sprintf("Mean: %.4f", mean(s_TX/s)))) +
    geom_hline(aes(yintercept=median(s_TX/s)), color="blue", lty="dashed") +
    geom_text(aes(x=2003, y=median(s_TX/s)*0.99,
                label=sprintf("Median: %.4f", median(s_TX/s)))) +
    geom_hline(yintercept=demand_data[year>2015, mean(s_TX/s)], color="red") +
    geom_text(data=demand_data[year>2015], aes(x=2010, y=mean(s_TX/s)*1.01,
                label=sprintf("Mean>2015: %.4f", mean(s_TX/s)))) +
    geom_hline(yintercept=demand_data[year>2015, median(s_TX/s)], color="red", lty="dashed") +
    geom_text(data=demand_data[year>2015], aes(x=2010, y=median(s_TX/s)*0.99,
                label=sprintf("Median>2015: %.4f", median(s_TX/s)))) +
    labs(title="Ratio of Texas production to national supply", y="Texas supply as % of total")
```

Output representative demand data
```{r}
fwrite(demand_data[outlier==FALSE & Date>="2010-01-01", .(p, q, s, s_TX), by=.(month,year)],
