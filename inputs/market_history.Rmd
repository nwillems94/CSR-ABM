...
```{r}
# identify outliers in three windows
demand_data[between(year, 1997, 2004),
                "outlier":= {p_limits= boxplot.stats(p)$stats; q_limits= boxplot.stats(q)$stats; # IQR
                            !between(p, min(p_limits), max(p_limits)) | !between(q, min(q_limits), max(q_limits))}]
demand_data[between(year, 2005, 2012),
                "outlier":= {p_limits= boxplot.stats(p)$stats; q_limits= boxplot.stats(q)$stats; # IQR
                            !between(p, min(p_limits), max(p_limits)) | !between(q, min(q_limits), max(q_limits))}]
demand_data[between(year, 2013, 2021),
                "outlier":= {p_limits= boxplot.stats(p)$stats; q_limits= boxplot.stats(q)$stats; # IQR
                            !between(p, min(p_limits), max(p_limits)) | !between(q, min(q_limits), max(q_limits))}]
...
```{r}
fwrite(demand_data[outlier==FALSE & year>=2010, .(p, q, q_TX), by=.(month,year)],
