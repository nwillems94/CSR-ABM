flaringABM_main <- function(Params, jobID, Run) {
    ...
    if (is.na(Params$refID)) {
        ...
        market_history <- data.table(
            "time"= Params$t0:Params$tf, key="time",
            "p_grey"= NA_real_, "p_green"= NA_real_,
            "p_oil_mult"= with(Params,
                rlnorm(1 + tf - t0, oil_price_mean, oil_price_sd)),
            "q_grey"= NA_real_, "q_green"= NA_real_, "q_oil"= NA_real_,
            "market_prop_green"= Params$market_prop_green,
            "RunID"= Run
        )

        # with what probability to exploring firms discover a new asset
        #    - validated based on EIA DPR
        Params$prob_e <- 1.23 / (Params$nagents * Params$prop_e *
                        leases[
                            is.na(firmID),
                            mean(oil_BBL + cond_BBL + (gas_MCF + csgd_MCF) / 6)
                        ])
    } else {
        demand <- readRDS(sprintf("./outputs/demand_function_%s-%s.rds",
                    Params$refID, Run))
        # update default arguments if necessary
        with(environment(demand$new_schedule),
        formals(new_schedule) <- c(alist(prop_green=, sample_set=list()),
                                        Params[c("p_low", "p_high")]))
        firms <- fread(sprintf("./outputs/agent_states_%s-%s.csv",
                            Params$refID, Run),
                        colClasses=list(
                            numeric=c("grey_gas_sold", "green_gas_sold",
                                    "cost_M", "sPressure"),
                            character="activity"))[
                    time == Params$t0 - 1]
        setkey(firms, firmID)

        leases <- fread(sprintf("./outputs/lease_states_%s-%s.csv",
                            Params$refID, Run),
                        colClasses=list(
                            integer="t_switch",
                            numeric="cost_csgd"))[
        ...
        leases[
            (class != "developed") | (status != "producing") |
        ...
        market_history <- fread(sprintf("./outputs/market_states_%s-%s.csv",
                            Params$refID, Run), colClasses="numeric")
        market_history[
            time >= Params$t0,
            c("p_grey", "p_green", "q_grey", "q_green", "q_oil"):= NA_real_
        ]
        market_history[
            time >= Params$t0,
            "market_prop_green":= Params$market_prop_green
        ...
        ref_params <- fread(sprintf("./logs/param_log_%s-%s.csv",
                                Params$refID, Run))
    ...
    if (Params$reporting != "accurate") {
        leases[
            capEx_csgd > 0,
            "opEx_pMCF":= opEx_pMCF - (612.5 * (capEx_csgd / 31250) / csgd_MCF)
        ]
        leases[
            !is.na(firmID) & (csgd_MCF > 0) & (class == "developed"),
            "t_switch":= NA_integer_
        ]
        leases[
            !is.na(firmID) & (csgd_MCF > 0) & (class == "developed"),
        ...
        if (Params$reporting == "misreported") {
            # assume flared casinghead volumes are mis-reported as captured
            #    and up to 3 times as much gas is actually flared
            leases[
                capEx_csgd > 0,
                "capEx_csgd":=
            ...
            # assume flared casinghead volumes are under-reported
            #    and 3 times as much gas is actually flared
            leases[
                capEx_csgd > 0,
                c("capEx_csgd", "sopf_MCF", "csgd_MCF"):=
                    {"flared_MCF"= csgd_MCF - 6 * (BOE_emp - oil_BBL);
        ...
        leases[
            !is.na(firmID) & (csgd_MCF > 0) & ((Params$t0 - t_found) >= 120),
            "class":= replace(class,
                (opEx_pMCF + (capEx_csgd / csgd_MCF / lifetime)) < 5, "developed")
        ]
        leases[
            !is.na(firmID) & (csgd_MCF > 0) & (class == "developed"),
            "t_switch":= t_found
        ...
        firms[
            leases[
                !is.na(firmID),
                .(sum(oil_BBL + cond_BBL), sum(gas_MCF),
                    sum(csgd_MCF[class == "underdeveloped"]) + sum(sopf_MCF)),
                by=firmID
                ], on="firmID",
            c("oil_output", "gas_output", "gas_flared"):= .(V1, V2, V3)]
        firms[,
            "behavior":= ifelse(gas_flared / oil_output > Params$threshold,
                "flaring", "economizing")
        ]
    }

    # unique random seed for consistent results across model runs
    seed_base <- market_history[,
        (Run * 10^floor(1 + log10(max(time) - min(time) + 1))) - min(time)
    ]
    ...
    for (ti in split(as.data.table(c("time"=list(Params$t0:Params$tf), Params)), by="time")) {
        ...
        firms[,
            "activity":= ifelse(runif(.N) < Params$prop_e,
        ...
        firms[
            leases[,
                .SD[
                    (status == "producing") & (class != "undeveloped"),
                    sum(oil_BBL + cond_BBL)
                ],
                by=firmID
            ], on="firmID",
            "oil_output":= V1
        ...
        firms[
            ...
            "gas_flared":= V1]
        firms[,
            "behavior":= ifelse((gas_flared == 0) |
                    ((gas_flared / oil_output) <= ti$threshold),
                ifelse(behavior == "flaring",
                    "economizing", behavior), "flaring")
        ]
        ...
        #    (revenue from oil + gas operations) -
        #    (baseline costs + additional costs spent on mitigation)
        firms[,
            "sales":= sales + oil_revenue + gas_revenue
        ]
        firms[,
            "profit":= profit + (oil_revenue + gas_revenue) - (cost_P + cost_M)
        ]
        ...
        firms[,
            "market_value":= (market_value / 2) +
                ((oil_revenue + gas_revenue - cost_P - cost_M) + # Income
                ((cost_M * ti$SRoR) - sPressure)) / 2            # Social value
        ]

        if (ti$time == min(market_history$time)) {
            firms[,
                "market_value":=
                    (oil_revenue + gas_revenue - cost_P - cost_M) +
                    ((cost_M * ti$SRoR) - sPressure)
            ]
        }
        ...
        #### AGENT RESPONSE ####
        # compare profit maximizing options
        #    with and without mitigation by comparing cost to possible harm
        cat("Options\n")
        portfolio_options <-
            optimal_strategy(firms, leases, market_history, demand_schedule, ti)

        #### OUTPUT STATES ####
        fwrite(firms,
            file=sprintf(agentOuts, Run), append=TRUE)
        fwrite(leases[(lifetime + t_found + 1) >= ti$time],
            file=sprintf(leaseOuts, Run), append=TRUE)
    }
