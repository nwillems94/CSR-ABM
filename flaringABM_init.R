...
generate_wells <- function(n) {
    wells <- data.table("wellID"=1:n)
...
generate_firms <- function(n, wells) {
    # initialize firms, none of whom are under social pressure or mitigating
    firms <- data.table("firmID"=1:n, "mitigation"=0, "sPressure"=0, "cash"=0, "market_value"=NA_real_)
