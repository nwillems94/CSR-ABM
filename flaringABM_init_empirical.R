...
leases_emp[model_csgd_MCF>0, "capEx_csgd":= 31250 * ceiling(flared_MCF/5475)]
...
leases <- leases_emp[, .(area, DISTRICT_NO, OIL_GAS_CODE,
                        ...
                        "BOE_emp"= model_oil_BBL + model_cond_BBL + (model_gas_MCF + model_csgd_MCF - flared_MCF)/6,
...
leases[!is.na(firmID) & (start<201001) & (expiration<201912),
        "t_found":= .SD[, lapply(.(expiration, 201001), function(x) 12*trunc(x/100) + x %% 100)][, 1+V1-V2]
                    - lifetime + Params$t0]
