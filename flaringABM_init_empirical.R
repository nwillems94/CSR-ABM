...
#       - Base: dollars based on the barrel of oil equivalent production (`opEx_pBOE`)
...
leases_emp <- fread("./inputs/processed/leases_emp.csv",
                    colClasses=c(LEASE_NO="character"))
wells <- unique(fread("./inputs/processed/wells.csv",
                    colClasses=c(LEASE_NO="character")))

## Estimate average production using decline curves
decline_params <- fread("./inputs/processed/decline_params.csv")
leases_emp[
    decline_params[, .(.(`Hyperbolic parameter`)), by=area], on="area",
    "param0":= sapply(V1, sample, 1)]
# match on counties where possible
leases_emp[
    decline_params, on=c("county", "area"),
    "param0":= `Hyperbolic parameter`]

# recalibrate initial production decline where necessary
leases_emp[
    !between(param, min(param0), max(param0)),
    "qdel_i":= optimize(function(x)
            (q_fit - eval(parse(text=decline_fun))(x, -m_shift + 1, param0))^2,
        c(0, q_i),
        tol=10
        )$minimum,
...
leases_emp[
    ...
    "param":= optimize(
        function(x)
            (q_fit - eval(parse(text=decline_fun))(qdel_i, -m_shift + 1, x))^2,
        c(0, 1)
    )$minimum,
    by=.(DISTRICT_NO, LEASE_NO)
]

# historical data range, maximally, from Jan 2010 - Dec 2019. Determine number
#    of additional months to estimate in order to average over entire model run
leases_emp[,
    "len_est":= .SD[, lapply(
        .(pmin(expiration, 201912), pmax(start, 201001)),
        function(x) 12 * trunc(x / 100) + x %% 100)
    ][,
    (V2 + (Params$tf - Params$t0)) - V1]
]
leases_emp[len_est < 0, "len_est":= 0]

# number of years to average over
leases_emp[,
    "len_avg":= (Params$tf - Params$t0) -
        ifelse(expiration < 201912, len_est - 1, 0)
]

# scale production to average over model runtime
leases_emp[
    (expiration >= 201912) & (len_est > 0) & (decline_fun != ""),
    "mult":= ((eval(parse(text=decline_fun))(qdel_i, len_est, param) +
            qcum_i_BBL + qcum_i_MCF / 6) / len_avg) /
        (total_oil_avg + cond_avg + total_MCF_avg / 6),
    by=.(DISTRICT_NO, LEASE_NO)
]

# project the Estimated Ultimate Recovery (EUR)
#    (over 360 months following EIA decline curves)
leases_emp[
    (expiration >= 201912) & (decline_fun != ""),
    "EUR":= (eval(parse(text=decline_fun))(qdel_i, 360, param) +
        (qcum_0_BBL + qcum_i_BBL + (qcum_0_MCF + qcum_i_MCF) / 6)),
    by=.(DISTRICT_NO, LEASE_NO)
]
#     no need to estimate production for leases which have all data
leases_emp[start <= 201001, "mult":= 1]
leases_emp[
    (expiration < 201912) | is.na(mult),
    c("mult", "EUR"):= .(1,
        (qcum_0_BBL + qcum_i_BBL) + (qcum_0_MCF + qcum_i_MCF) / 6)
]

leases_emp[,
    "EUR_MCF":= 6 * (EUR * ((csgd_avg + gas_avg) / 6) /
        (oil_avg + cond_avg + (csgd_avg + gas_avg) / 6))
]
leases_emp[,
    "EUR_BBL":=     (EUR * (oil_avg + cond_avg) /
        (oil_avg + cond_avg + (csgd_avg + gas_avg) / 6))
]
...
leases_emp[, c("model_oil_BBL", "model_cond_BBL",
            "model_gas_MCF", "model_csgd_MCF", "flared_MCF"):=
            .(oil_avg,          cond_avg,
            gas_avg,            csgd_avg,       flared_MCF_avg)]

leases_emp[
    total_MCF_avg > 0,
    "model_gas_MCF":= pmin(EUR_MCF / len_avg, gas_avg * mult)
]
leases_emp[
    total_MCF_avg > 0,
    "model_csgd_MCF":= pmin(EUR_MCF / len_avg, csgd_avg * mult)
]
leases_emp[
    total_oil_avg > 0,
    "model_oil_BBL":= pmin(EUR_BBL / len_avg, oil_avg * mult)
]

leases_emp[
    total_MCF_avg > 0,
    "flared_MCF":= (model_gas_MCF + model_csgd_MCF) *
        flared_MCF_avg / (gas_avg + csgd_avg)
]
leases_emp[(model_gas_MCF + model_csgd_MCF) == 0, "flared_MCF":= 0]

# quantity of gas flared for safety and operational reasons
leases_emp[
    (total_MCF_avg > 0) & (OIL_GAS_CODE == "G"),
    "sopf_MCF":= flared_MCF
]
...
leases_emp[
    leases_emp[
            model_gas_MCF > 0,
            sum(sopf_MCF) / sum(model_gas_MCF),
            by=area],
        on="area",
    "sopf_MCF":= ifelse((total_MCF_avg > 0) & (OIL_GAS_CODE == "O"),
        model_csgd_MCF * V1, sopf_MCF)
]
...
wells[
    area == "Eagle Ford",
    "capExMM":= sort(rlnorm(.N, log(7.6 * 5.5) / 2,
                        log(7.6 / 5.5) / 1.282 / 2))
]
setorder(wells, -shared, depth)
wells[
    area == "Delaware Basin",
    "capExMM":= sort(rlnorm(.N, log(8.5 * 5.0) / 2,
                        log(8.5 / 5.0) / 1.282 / 2))
]
wells[
    area == "Midland Basin",
    "capExMM":= sort(rlnorm(.N, log(8.6 * 5.5) / 2,
                        log(8.6 / 5.5) / 1.282 / 2))
]
# only the average (2.5) is given for Spraberry.
#    Assume a coefficient of variation of 0.2
wells[
    area == "Spraberry",
...
# apply a price index to convert from 2014 dollars to historical costs
#    (which were mostly lower)
...
wells[
    (year < min(well_cost_index$year)) | is.na(start),
    "capExMM":= capExMM * well_cost_index[which.min(year), real_index_2014]
]

# stacked multilaterals have one cost
wells[
    !leases_emp, on=c("DISTRICT_NO", "LEASE_NO"),
    "capExMM":= replace(capExMM, !is.na(stacked), 0)
]

# wells drilled after the peak production time do not contribute to the EUR used here
wells[
    leases_emp[expiration >= 201912], on=c("DISTRICT_NO", "LEASE_NO"),
    "capExMM":= replace(capExMM, if (isTRUE(all(start > t_i))) -which.max(start)
        else (start > t_i), 0), by=.EACHI
]
...
leases_emp[
    wells[
        !is.na(start) & (capExMM > 0),
        .(sum(capExMM), .N), by=.(DISTRICT_NO, LEASE_NO)
        ], on=c("DISTRICT_NO", "LEASE_NO"),
    c("capEx", "N"):= .(V1 * 10^6, N)
]

leases_emp[,
    "capEx":= replace(capEx, is.na(N) & (start < 201910),
        min(capEx, na.rm=TRUE)),
    by=area
]
...
leases_emp[ # oil
    (area == "Eagle Ford") & (oil_avg > 0 | ((cond_avg > 0) & (gas_avg == 0))),
    "opEx_pBOE":= sort(rlnorm(.N, log(15.2 * 9.3) / 2,
                            log(15.2 / 9.3) / 1.282 / 2))
]
leases_emp[ # wet gas
    (area == "Eagle Ford") & (gas_avg > 0) & (cond_avg > 0),
    "opEx_pBOE":= sort(rlnorm(.N, log(13.5 * 6.7) / 2,
                            log(13.5 / 6.7) / 1.282 / 2))
]
leases_emp[ # dry gas
    (area == "Eagle Ford") & 
        (((gas_avg > 0) & (cond_avg == 0)) | ((csgd_avg > 0) & (oil_avg == 0))),
    "opEx_pBOE":= sort(rlnorm(.N, log(10.3 * 5.3) / 2,
                            log(10.3 / 5.3) / 1.282 / 2))
]
leases_emp[
    area == "Delaware Basin",
    "opEx_pBOE":= sort(rlnorm(.N, log(16.1 * 7.7) / 2,
                            log(16.1 / 7.7) / 1.282 / 2))
]
leases_emp[
    area %in% c("Midland Basin", "Spraberry"),
    "opEx_pBOE":= sort(rlnorm(.N, log(16.1 * 7.9) / 2,
                            log(16.1 / 7.9) / 1.282 / 2))
]



# oil (pBBL):   Short Transportation, Long Transportation
#    (ordered by how oil moves & distance from crude pipelines)
# cond (pBBL):  Long Transportation, NGL fractionation
#    (ordered by how oil moves & distance from crude pipelines)
cat("\tAssigning lease operating expenses per barrel of oil\n\t")
setorder(leases_emp, -pipe_frac, -rail_frac, crude_dist)

leases_emp[
    (area == "Eagle Ford") & (oil_avg > 0),
    "opEx_pBBL":= sort(rlnorm(.N, log(6 * 3.75) / 2,
                            log(6 / 3.75) / 1.282 / 2))
]
leases_emp[
    (area == "Eagle Ford") & (cond_avg > 0),
    "opEx_pBBL":= sort(rlnorm(.N, log(5.64 * 4.72) / 2,
                            log(5.64 / 4.72) / 1.282 / 2))
]
leases_emp[
    (area == "Delaware Basin") & (oil_avg > 0),
    "opEx_pBBL":= sort(rlnorm(.N, log(16 * 4.25) / 2,
                            log(16 / 4.25) / 1.282 / 2))
]
leases_emp[
    (area == "Delaware Basin") & (cond_avg > 0),
    "opEx_pBBL":= sort(rlnorm(.N, log(13.78 * 6.13) / 2,
                            log(13.78 / 6.13) / 1.282 / 2))
]
leases_emp[
    area %in% c("Midland Basin", "Spraberry") & (oil_avg > 0),
    "opEx_pBBL":= sort(rlnorm(.N, log(15.5 * 4.25) / 2,
                            log(15.5 / 4.25) / 1.282 / 2))
]
leases_emp[
    area %in% c("Midland Basin", "Spraberry") & (cond_avg > 0),
    "opEx_pBBL":= sort(rlnorm(.N, log(13.38 * 5.29) / 2,
                            log(13.38 / 5.29) / 1.282 / 2))
]

leases_emp[oil_avg + cond_avg == 0, "opEx_pBBL":= 0]


# gas (pMCF):   Gathering & Transportation, Processing
#    (inverse-square distance weighted gas processing capacity)
...
leases_emp[ # oil
    (area == "Eagle Ford") & (csgd_avg > 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(1.60 * 0.85) / 2,
                            log(1.60 / 0.85) / 1.282 / 2))
]
leases_emp[ # wet gas
    (area == "Eagle Ford") & (gas_avg > 0) & (cond_avg > 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(1.60 * 0.85) / 2,
...
leases_emp[ # dry gas
    (area == "Eagle Ford") & (gas_avg > 0) & (cond_avg == 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(1.05 * 0.55) / 2,
                            log(1.05 / 0.55) / 1.282 / 2))
]

leases_emp[ # oil
    (area == "Delaware Basin") & (csgd_avg > 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(2.35 * 0.85) / 2,
                            log(2.35 / 0.85) / 1.282 / 2))
]
leases_emp[ # wet gas
    (area == "Delaware Basin") & (gas_avg > 0) & (cond_avg > 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(2.35 * 0.85) / 2,
                            log(2.35 / 0.85) / 1.282 / 2))
]
leases_emp[ # dry gas
    (area == "Delaware Basin") & (gas_avg > 0) & (cond_avg == 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(1.10 * 0.60) / 2,
                            log(1.10 / 0.60) / 1.282 / 2))
]

leases_emp[ 
    area %in% c("Midland Basin", "Spraberry") & (csgd_avg > 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(1.70 * 0.85) / 2,
                            log(1.70 / 0.85) / 1.282 / 2))
]
leases_emp[ # wet gas
    area %in% c("Midland Basin", "Spraberry") &
        (gas_avg > 0) & (cond_avg > 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(1.70 * 0.85) / 2,
                            log(1.70 / 0.85) / 1.282 / 2))
]
leases_emp[ # dry gas
    area %in% c("Midland Basin", "Spraberry") &
        (gas_avg > 0) & (cond_avg == 0),
    "opEx_pMCF":= sort(rlnorm(.N, log(0.90 * 0.60) / 2,
                            log(0.90 / 0.60) / 1.282 / 2))
]

# each compressor costs $7,350 per year to operate
leases_emp[
    model_csgd_MCF > 0,
    "opEx_pMCF":= opEx_pMCF + (612.5 * (capEx_csgd / 31250) / model_csgd_MCF)
]
...
# estimate lifetime based on
#    initial production rate at t_i and total production since t_i
leases_emp[
    (expiration >= 201912) & (decline_fun != ""),
    "q_curve":= (eval(parse(text=decline_fun))(qdel_i, -m_shift + 1, param) +
        EUR - (qcum_0_BBL + qcum_i_BBL + (qcum_0_MCF + qcum_i_MCF) / 6)),
    by=.(DISTRICT_NO, LEASE_NO)
]
...
leases_emp[,
    "m_rem":= .SD[,
            lapply(.(expiration, t_i), function(x)
                12 * trunc(x / 100) + x %% 100)
            ][,
            1 + V1 - V2
            ]
]
leases_emp[
    order(-log(q_i / q_curve) * runif(.N, 0.5, 1.5)),
    "m_rem":= ifelse(expiration < 201912, m_rem,
        sort(sample(m_rem[(start > 199301) & (expiration < 201912)],
            .N, replace=TRUE))),
    by=area
]
...
leases_emp[,
    "lifetime":= .SD[,
        lapply(.(t_i, start, expiration), function(x)
            12 * trunc(x / 100) + x %% 100)
        ][,
...
firms <- data.table(
    "firmID"= 1:Params$nagents, key="firmID",
    ...
    "grey_gas_sold"= NA_real_, "green_gas_sold"= NA_real_,
    "gas_flared"= NA_real_,
    ...
    # activities: exploration, development;
    # behaviors: flaring, mitigating, economizing, imitating
    "activity"= NA_character_, "behavior"= NA_character_,
    "time"= Params$t0 - 1
)

# Assign firms maximum production capacity based on empirics
#    (oil production is log-normally distributed)
...
while (!between(sum(firms$production_BBL) / mean(market_shares[, sum(scaled_oil_BBL), by=.(CYCLE_YEAR, CYCLE_MONTH)]$V1), 0.7, 0.75)) {
    firms[,
        "production_BBL":= sort(rlnorm(
            .N, mean(log(market_shares[scaled_oil_BBL > 0, scaled_oil_BBL])),
            sd(log(market_shares[scaled_oil_BBL > 0, scaled_oil_BBL]))
    ))]
}

for (ID in firms[order(production_BBL)]$firmID) {
    rem_capacity <- firms[ID, production_BBL] -
        leases_emp[firmID == ID, sum(total_oil_avg)]
    leases_emp[
        is.na(firmID) & (oil_avg > 0) & (total_oil_avg < rem_capacity),
        "firmID":= replace(firmID, sample(.N,
            min(1 + rem_capacity / mean(total_oil_avg), .N),
            prob=1 / capEx), ID)
    ]
    if (leases_emp[firmID == ID, sum(oil_avg > 0)] == 0) {
        leases_emp[
            is.na(firmID) & (oil_avg > 0),
            "firmID":= replace(firmID, sample(.N, 1, prob=1 / capEx), ID)
        ]
    }
}
...
while (!between(sum(firms$production_MCF) / mean(market_shares[, sum(scaled_gas_MCF), by=.(CYCLE_YEAR, CYCLE_MONTH)]$V1), 0.7, 0.75)) {
    firms[
        order(log(production_BBL) * runif(.N, 0.5, 1.5)),
        "production_MCF":= sort(rlnorm(
            .N, mean(log(market_shares[scaled_gas_MCF > 0, scaled_gas_MCF])),
            sd(log(market_shares[scaled_gas_MCF > 0, scaled_gas_MCF]))
        ))
    ]
    # a large fraction (~40%) of firms operate oil leases only
    firms[
        runif(.N) < market_shares[,
                        sum(scaled_gas_MCF == 0) / length(scaled_gas_MCF)
                    ],
        "production_MCF":= 0
    ]
}
...
for (ID in firms[production_MCF > 0][order(production_MCF)]$firmID) {
    rem_capacity <- firms[ID, production_MCF] -
        leases_emp[(firmID == ID) & (gas_avg > 0), sum(total_MCF_avg)]

    leases_emp[
        is.na(firmID) & (gas_avg > 0) & (total_MCF_avg < rem_capacity),
        "firmID":= replace(firmID, sample(.N,
                min(1 + rem_capacity / mean(total_MCF_avg), .N),
            prob=1 / capEx), ID)
    ]

    if (leases_emp[firmID == ID, sum(gas_avg > 0)] == 0) {
        leases_emp[
            is.na(firmID) & (gas_avg > 0),
            "firmID":= replace(firmID, sample(.N, 1, prob=1 / capEx), ID)
        ]
    }
}
...
leases <- leases_emp[,
    .(area, DISTRICT_NO, OIL_GAS_CODE,
    "oil_BBL"= model_oil_BBL, "cond_BBL"= model_cond_BBL,
    "gas_MCF"= model_gas_MCF, "csgd_MCF"= model_csgd_MCF,
    sopf_MCF, ERR_MCF, EUR,
    # empirical BOEs sold to calculate the baseline operating cost
    "BOE_emp"= model_oil_BBL + model_cond_BBL +
        (model_gas_MCF + model_csgd_MCF - flared_MCF) / 6,
...
leases[
    !is.na(firmID),
    "t_found":= Params$t0 - sample(lifetime - 1, 1),
    by=leaseID
]
# set number of months of operation
leases[
    !is.na(firmID) & (start < 201001) & (expiration < 201912),
    "t_found":= .SD[,
        lapply(.(expiration, 201001), function(x) 12 * trunc(x / 100) + x %% 100)
        ][,
        1 + V1 - V2
        ] - lifetime + Params$t0
]
...
leases[!is.na(firmID), "lifetime":= lifetime + 2]
# offset for first time step
leases[
    !is.na(firmID) & (ERR_MCF > 0),
    "ERR_MCF":= ERR_MCF + (gas_MCF + csgd_MCF)
]

leases[
    !is.na(firmID),
    c("class", "status", "market"):=
        .(ifelse(csgd_MCF > 0, "underdeveloped", "developed"),
            "producing", "grey")
]
# historically (2004-2010), gas prices were much higher (~5/MCF).
#    Develop leases accordingly
leases[
...
firms[, "behavior":= ifelse(gas_flared / oil_output > Params$threshold,
                            "flaring", "economizing")]
...
historical_market_data[
    market_shares[,
        sum(OPER_GAS_PROD_VOL + OPER_CSGD_PROD_VOL),
        by=.("year"=CYCLE_YEAR, "month"=month.abb[CYCLE_MONTH])
    ], on=c("year", "month"),
    "frac":= leases_emp[
            !is.na(firmID),
            sum(model_gas_MCF + model_csgd_MCF - flared_MCF)
        ] / V1
]

# create an object which encapsulates the necessary demand data
#    and generates demand schedules
demand <-
...
cat(gsub("Time difference of", "Initialization complete in",
    capture.output(Sys.time() - init_time)),
    "\n")
