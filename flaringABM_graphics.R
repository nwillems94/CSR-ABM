...
log1p_breaks <- function(x, n) {
    replace(axisTicks(c(0, log10(max(x))), log=TRUE, n=n), 1, 0)
}
...
colors <- dbGetQuery(db,
    sprintf(lookup_sql, "string_key AS model, integer_key",
                        "model",
                        "integer_key<12 ORDER BY integer_key"))
setDT(colors)

colors[
    model %in% c("complete", "CA market", "target top", "under-reported"),
    "color":= "#000000"
]
colors[
    is.na(color),
    "color":= palette.colors(.N, palette="Tableau 10")
]
colors[, "color":= replace(color, model == "CA market",
    rgb(colorRamp(c(color[model == "EU market"], 1))(0.5) / 255))]
colors[, "color":= replace(color, model == "target top",
    rgb(colorRamp(c(color[model == "target even"], 1))(0.5) / 255))]
colors[, "color":= replace(color, model == "under-reported",
    rgb(colorRamp(c(color[model == "mis-reported"], 1))(0.5) / 255))]
...
for (col in c("model", "strategy", "reporting")) {
    ...
    params[, c(col):= as.character(dbGetQuery(db, sprintf(lookup_sql,
                                    "string_key", col,
                                    paste0("integer_key=", .BY)))), by=get(col)]
}
params[, "model":= factor(model,
                    intersect(unique(model), names(model_colors)))]
...
market_history[, "model":= as.character(dbGetQuery(db, sprintf(lookup_sql,
                                        "string_key", "model",
                                        paste0("integer_key=", .BY)))), by=model]
market_history[, "model":= factor(model,
                            intersect(unique(model), names(model_colors)))]
...
##### INITIALIZATION #####
sql <- "SELECT * FROM %2$s 
        LEFT JOIN %1$s ON %2$s.leaseID=%1$s.leaseID 
...
ggplot(leases[gas_MCF + csgd_MCF > 0],
    aes(x=opEx_pMCF +
        ifelse(csgd_MCF == 0, 0, capEx_csgd / csgd_MCF / lifetime))) +
    stat_density(geom="line",
        aes(lty=ifelse(OIL_GAS_CODE == "O", "Oil lease", "Gas lease")),
...
ggplot(leases[oil_BBL > 0], aes(color=area)) +
    geom_density(data= function(x)
            {x[,"area":= ifelse(area == "Spraberry", "Midland Basin", area)]},
        aes(x=cost_oil / oil_BBL, color=area), lwd=1) +
    geom_point(aes(x=Mean, y=(0.5 + (year == 2021))), data=fed_data) +
    geom_errorbarh(aes(xmin=Min, xmax=Max, y=(0.5 + (year == 2021))),
        alpha=0.8, lty=2, lwd=1, data=fed_data, show.legend=FALSE) +
    annotate("text", x=fed_data[year == 2017, max(Mean)], y=0.5,
        label="2017", alpha=0.7, vjust=-1) +
    annotate("text", x=fed_data[year == 2021, min(Mean)], y=1.5, label="2021",
        alpha=0.7, vjust=-1) +
    scale_x_log10() +
    labs(title="Oil operating costs at oil leases", x="$ / barrel", color="") +
    theme_custom() + 
    theme(legend.position=c(0.85, 0.8), legend.title=element_blank())
dev.off()
rm(fed_data)
...
ggplot(melt(market_shares,
        id.vars="scaled_oil_BBL",
        measure.vars=patterns("_MCF$"),
        variable.name="type")) +
    stat_density_2d(geom="raster", contour=FALSE,
        aes(x=scaled_oil_BBL, y=value, fill=after_stat(density))) +
    stat_bin_2d(geom="point", alpha=0.5,
        aes(x=V1, y=V2, size=after_stat(count), color="red", fill=NULL),
        data=leases[
            t_found <= time,
            .(sum(oil_BBL), c(sum(gas_MCF), sum(csgd_MCF)),
                "type"=c("scaled_gas_MCF","scaled_csgd_MCF")),
            by=.(RunID, firmID)
        ]) +
    ...
    scale_x_continuous(trans=scales::log1p_trans(),
        breaks= function(x) {log1p_breaks(x, 5)}) +
    scale_y_continuous(trans=scales::log1p_trans(),
        breaks= function(x) {log1p_breaks(x, 5)}) +
    labs(title="Joint distribution of firm oil and gas production",
        x="Production (BBL)", y="Production (MCF)") +
    facet_grid(.~type, labeller=as_labeller(function(x)
        ifelse(x == "scaled_gas_MCF", "Unassociated Gas", "Casinghead Gas"))) +
    theme_custom() +
...
sql <- "SELECT %1$s.gas_MCF, %1$s.csgd_MCF, %1$s.opEx_pMCF, 
        %2$s.market, %2$s.RunID, %2$s.model FROM %2$s 
        INNER JOIN %1$s ON %2$s.leaseID=%1$s.leaseID 
            AND %1$s.RunID=%2$s.RunID 
            AND %1$s.model=%2$s.model 
...
gas_supply <- dbGetQuery(db, sprintf(sql, "lease_info", "lease_states",
                                sprintf(lookup_sql, "integer_key", "market",
                                    "string_key IN ('green','grey')"),
...
gas_supply[
    market_history, on=c("model","RunID"),
    "max_prop_green":= market_prop_green
]

gas_demand <- unserialize(dbGetQuery(db,
        "SELECT * FROM demand_functions ORDER BY RANDOM()")$fun_bin[[1]])

gas_supply[, c("p_grey","p_green"):=
    gas_demand$new_schedule(first(max_prop_green))(q_sup)[, .(p_grey, p_green)]]
...
ggplot(gas_supply[,
            if (any((p_green < opEx_pMCF) & (market == "green"))) .SD,
            by=.(model, RunID)
            ][RunID == min(RunID)],
        aes(x=q_sup, color=market)) +
    geom_line(aes(y=opEx_pMCF), size=0.85) +
    geom_line(aes(y=p_grey, color="grey"), linetype="dashed", size=0.85) +
    geom_line(aes(y=replace(p_green, p_green == 0, NA), color="green"),
        linetype="dashed", size=0.85) +
    scale_color_manual(values=c("grey"="grey40", "green"="darkgreen")) +
    scale_x_continuous(labels= function(x) format(x, scientific=TRUE)) +
    labs(title="Sample Natural Gas Market",
        x="Quantity (MCF / month)", y="Price ($)", color="Market") +
    theme_custom() + theme(legend.position=c(0.85,0.2))
dev.off()


# flaring intensity
sql <- "SELECT SUM(gas_flared)/SUM(oil_output) AS oil,
            SUM(gas_flared)/SUM(gas_output) AS gas, 
            %2$s.model, %2$s.RunID, %2$s.time FROM %2$s 
        INNER JOIN %1$s ON %2$s.firmID=%1$s.firmID 
            AND %1$s.RunID=%2$s.RunID 
            AND %1$s.model=%2$s.model 
            AND %2$s.model IN (%3$s) 
        GROUP BY %2$s.model, %2$s.RunID, %2$s.time"
intensity_sql <- sprintf(sql, "agent_info", "agent_states",
    sprintf(lookup_sql, "integer_key", "model", "%1$s"))

flaring_intensity <- dbGetQuery(db,
    sprintf(intensity_sql, "string_key='complete'"))
...
flaring_intensity[, "model":= as.character(dbGetQuery(db, sprintf(lookup_sql,
                                "string_key", "model",
                                paste0("integer_key=", .BY)))), by=model]
flaring_intensity[, "model":= factor(model,
                                intersect(unique(model), names(model_colors)))]

svglite("./graphics/market_intensity.svg", height=7, width=12, bg="transparent")
ggplot(melt(flaring_intensity,
        measure.vars=c("gas","oil"), variable.name="basis"),
        aes(x=time, y=value)) +
    ...
    geom_hline(aes(yintercept=y), lty=2, 
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    labs(title="Market evolution",
        x="Time", y="Total Flaring Intensity", color="") +
...
ggplot(melt(flaring_intensity[time >= -1],
        measure.vars=c("gas","oil"), variable.name="basis"),
        aes(x=time, y=value)) +
    geom_line(aes(group=RunID), alpha=0.25) +
    stat_summary(geom="line", fun="mean", lwd=1) +
...
boxplot(market_history[(time < 0) & (model == "complete")]$p_grey,
    add=TRUE, boxwex=1.5, at=2, yaxt="n")
...
boxplot(monthly_prices[year > 2010]$p, boxwex=1.6, add=TRUE, at=1, yaxt="n")
axis(1, at=1, labels="empirical", col.axis="blue", cex.axis=18 / 11)
boxplot(p_grey~as.character(model),
    data=market_history[(time < 0) & (model %in% warm_models)],
...
# density of firms
agent_end_states <- dbGetQuery(db, "SELECT market_value, profit, 
                                        oil_output, gas_output, gas_flared, 
                                        behavior, model, RunID 
                                    FROM agent_states 
...
agent_end_states[, "model":= factor(model,
                                intersect(unique(model), names(model_colors)))]

## market value
svglite("./graphics/complete_density.svg", height=7, width=12, bg="transparent")
ggplot(agent_end_states[(model == "complete") & (gas_flared > 0)],
        aes(x=market_value, y=gas_flared / oil_output)) +
    geom_density_2d_filled(show.legend=FALSE) +
    geom_hline(yintercept=0.05, color="white", linetype=2, lwd=1) +
    scale_x_continuous(trans=scales::log1p_trans(),
        breaks= function(x) {log1p_breaks(x, 5)}) +
    scale_y_log10() +
    annotate("text", x=100, y=0.05,
        label="Certified market\nthreshold", color="white") +
    labs(title="Density of all firms",
        x="Market Value", y="Flaring Intensity") +
    theme_custom() + coord_cartesian(expand=FALSE)
dev.off()

svglite("./graphics/complete_distribution.svg", height=7, width=12, bg="transparent")
ggplot(agent_end_states[(behavior!="flaring") & (model == "complete")],
        aes(x=market_value)) +
    stat_density(aes(color=behavior), geom="line", position="identity") +
    scale_x_continuous(trans=scales::log1p_trans(),
        breaks= function(x) {log1p_breaks(x, 5)}) +
    labs(title="Density of non flaring firms",
        x="Market Value", color="Behavior:") +
    theme_custom() + theme(legend.position=c(0.85,0.8)) +
    coord_cartesian(expand=FALSE)
dev.off()
...
ggplot(agent_end_states[,
        sum(BOE_output[profit < 0]) / sum(BOE_output),
        by=.(RunID, model)
    ]) +
    geom_boxplot(aes(x=V1, y=model, color=model), show.legend=FALSE) +
    scale_x_continuous(labels=scales::percent) +
...
agent_states <- dbGetQuery(db,
    sprintf("SELECT market_value, grey_gas_sold, green_gas_sold, 
...
agent_states[, "model":= factor(model,
                            intersect(unique(model), names(model_colors)))]

agent_states[
    market_history, on=c("model","RunID","time"),
...
ggplot(agent_states) +
    geom_violin(draw_quantiles=0.5,
        aes(x=behavior, y=gas_revenue / (grey_gas_sold + green_gas_sold),
        color=model)) +
    scale_x_discrete(limits=c("flaring", "economizing", "mitigating", "imitating")) +
    scale_color_manual(values=model_colors) +
    labs(title="Firm earnings for gas",
        x="Behavior", y="Earnings per MCF", color="") +
...
ggplot(agent_states[
            ...
            Ci / MV0i / cut(Ci / SRoR / MV0i, 10, labels=FALSE)},
            by=.(RunID, time)
        ]) +
    geom_boxplot(aes(x=as.factor(RunID), y=V1),
        outlier.shape=NA, show.legend=FALSE) +
    geom_hline(yintercept=0.18 * 0.19, lty=2) +
    geom_hline(yintercept=0.11 + (0.18 * 0.19), lty=2) +
    labs(title="Effect of shareholder valuation on market value",
        x="RunID", y="% Increase in Market Value") +
...
ggplot(market_history[!is.na(q_stored) & (model == "complete")][
        order(time),
        .(time, "net"= cumsum(q_stored), "frac"= mean(frac)),
        by=.(model, RunID
    )], aes(x=time, y=net)) +
    geom_point(aes(group=RunID), alpha=0.25) +
    stat_summary(geom="line", fun="mean") +
    stat_summary(geom="ribbon", fun.data="mean_se", alpha=0.5) +
    geom_hline(aes(yintercept=mean(frac) * 400000 * 1000),
        color='darkred', linetype='dashed') +
    annotate("text", x=0, y=0.95 * 400000 * 1000 * mean(market_history$frac),
            label="Scaled Working Gas Storage",
            size=14 / .pt, color='darkred', hjust=1) +
...
ggplot(market_history[(p_green > 0) & (model == "complete")],
        aes(x=time, y=p_green / p_grey, group=RunID)) +
    ...
    labs(title="Certified gas premium",
        y=expression(P[green] / P[grey]), color="RunID") +
...
ggplot(market_history[(p_green > 0) & (model %in% c("complete","CA market", "EU market"))], 
        aes(x=time, y=p_green / p_grey, group=RunID)) +
    geom_point(alpha=0.25) +
    geom_rug(alpha=0.25, color="grey40", sides='r', outside=TRUE) +
    coord_cartesian(clip='off') +
    ...
    labs(title="Certified gas premium",
        y=expression(P[green] / P[grey]), color="RunID") +
    theme_custom() +
    theme(strip.switch.pad.grid=unit(0.4,'cm'), strip.placement='outside')
dev.off()
...
ggplot(market_history[(time > 0) & model %in% c("complete","CA market", "EU market"),
    ...
    scale_x_discrete(breaks=c("p_grey","p_green"),
        labels=c(expression(P[grey]), expression(P[green]))) +
    scale_color_manual(values=model_colors) +
...
ggplot(market_history[model %in% c("complete", "no differentiation")],
        aes(x=time, y=q_green + q_grey)) +
    geom_point(aes(group=RunID), alpha=0.25) +
    stat_summary(geom="line", fun="mean") +
    stat_summary(geom="ribbon", fun.data="mean_se", alpha=0.5) +
    facet_grid(model~.) +
...
ggplot(melt(flaring_intensity,
    ...
    scale_color_manual(values=model_colors) +
    scale_fill_manual(values=model_colors, guide=NULL) +
    geom_vline(xintercept=0, lty=1, color="grey") +
    annotate(x=0, y=Inf, "text", label="Warm start", hjust=1.1, vjust=1.5) +
    annotate(x=0, y=Inf, "text", label=" Forecast", hjust=-0.1, vjust=1.5) +
    geom_hline(aes(yintercept=y), lty=2,
...
main <-
ggplot(melt(flaring_intensity[time < 0],
            measure.vars=c("gas","oil"), variable.name="basis"),
        aes(x=time, y=value)) +
    stat_summary(geom="line", fun="mean", lwd=1, aes(color=model)) +
    stat_summary(geom="ribbon", fun.data="mean_se", aes(fill=model), alpha=0.25) +
    ...
    geom_hline(aes(yintercept=y), lty=2,
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    labs(title="Market equilibration", subtitle="Warm-start period",
        x="Time", y="Total Flaring Intensity", color="") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() + theme(legend.position="bottom")
...
ggplot(flaring_intensity[between(time, -30, 0)], aes(x=time)) +
    stat_summary(geom="line", fun="mean", lwd=1, aes(color=model)) +
    stat_summary(geom="ribbon", fun.data="mean_se", aes(fill=model), alpha=0.25) +
    scale_color_manual(values=model_colors, guide=NULL) +
    scale_fill_manual(values=model_colors, guide=NULL) +
...
layer(data= data.table(basis="gas", value=0, time=-40),
        ...
        params=list(xmin=-50, xmax=0,
                    ...
                    grob=ggplotGrob(inset + aes(y=gas) +
                        geom_hline(yintercept=c(0.05, 0.03), lty=2)))
    )
...
layer(data= data.table(basis="oil", value=0, time=-40),
        ...
        params=list(xmin=-50, xmax=0,
                    ...
                    grob=ggplotGrob(inset + aes(y=oil) +
...
################################## COMPONENTS ###################################
intensity_sql <- sub("GROUP BY", 
                    "    AND agent_states.time>=-1 \n\t    GROUP BY",
                    intensity_sql)
...
ggplot(melt(flaring_intensity,
    ...
    geom_hline(aes(yintercept=y), lty=2,
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    labs(title="Impact of market components",
        x="Time", y="Total Flaring Intensity", color="") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() + theme(legend.position="bottom")
dev.off()
...
ggplot(melt(flaring_intensity,
    ...
    geom_hline(aes(yintercept=y), lty=2,
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    labs(title="Impact of activist targeting strategy",
        x="Time", y="Total Flaring Intensity", color="") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() + theme(legend.position="bottom")
dev.off()

flaring_intensity_der <- melt(flaring_intensity, 
                            measure.vars=c("gas","oil"),
                            variable.name="basis")
flaring_intensity_der[,
    "der":= predict(smooth.spline(time, y=value), time, deriv=1)$y,
    by=.(RunID, model)
]
...
ggplot(flaring_intensity_der[!is.na(der) & (time >= -1)], aes(x=time, y=der)) +
    ...
    labs(title="Impact of activist targeting strategy",
        x="Time", y="Derivative estimate of flaring intensity", color="") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() + theme(legend.position="bottom")
dev.off()

# add annotations of flaring thresholds
labels <- flaring_intensity_der[
            between(time, 10, 50),
            .("y"=mean(value)), 
            by=.(model, time, basis)
        ]
...
ggplot(melt(flaring_intensity,
    ...
    geom_label_repel(data=labels, aes(x=time, y=y, label=label, fill=model),
                    color=ifelse(labels$model %in% c("complete","target top"),
                        "white", "black"),
                    nudge_x=ifelse(labels$model %in% c("complete", "target top"),
                        -10, 10),
                    box.padding=1.5, max.overlaps=50, size=5,
                    segment.curvature=1e-20, segment.color="grey40") +
    labs(title="Impact of activist targeting strategy",
        subtitle="Forecast period", x="Time", y="Value") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() +
...
ggplot(melt(flaring_intensity[model!="no differentiation"],
            measure.vars=c("gas","oil"), variable.name="basis"),
        aes(x=time, y=value)) +
    stat_summary(geom="line", fun="mean", lwd=1, aes(color=model)) +
    stat_summary(geom="ribbon", fun.data="mean_se", aes(fill=model), alpha=0.25) +
    ...
    geom_hline(aes(yintercept=y), lty=2,
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    labs(title="Impact of certified market threshold",
        x="Time", y="Total Flaring Intensity", color="") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() + theme(legend.position="bottom")
dev.off()
...
# add annotations of flaring thresholds
labels <- melt(flaring_intensity[between(time, 10, 50)],
            measure.vars=c("gas","oil"), variable.name="basis")[,
            .("y"=mean(value)), by=.(model, time, basis)]

labels[
    params[, unique(threshold), by=model], on="model",
    "label":= sprintf("Threshold of\n%s MCF/BBL", V1)
]
labels[time!=30, "label":= ""]
labels[
    !(((basis == "oil") & (model == "lower threshold")) |
        ((basis == "oil") & (model == "complete"))),
    "label":= ""
]
labels[
    (basis == "gas") & (time == 30) & (model == "no differentiation"),
    "label":= "No certified market"
]
...
ggplot(melt(flaring_intensity,
    ...
    geom_hline(aes(yintercept=y), lty=2,
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    geom_label_repel(data=labels, aes(x=time, y=y, label=label, fill=model),
                    color=ifelse(labels$model == "complete",
                        "white", "black"),
                    nudge_y=ifelse(labels$model == "no differentiation",
                        0.01, 0),
                    nudge_x=ifelse(labels$model == "complete",
                        -10, 10),
                    box.padding=1.5, max.overlaps=50, size=5,
                    segment.curvature=1e-20, segment.color="grey40") +
    labs(title="Impact of certified market threshold",
        subtitle="Forecast period", x="Time", y="Total Flaring Intensity") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom()
dev.off()
...
ggplot(melt(flaring_intensity,
    ...
    geom_hline(aes(yintercept=y), lty=2,
        data=data.table("y"=c(0.1, 0.05, 0.03), "basis"=c("oil","gas","gas"))) +
    labs(title="Impact of certified market size",
        x="Time", y="Total Flaring Intensity", color="") +
    facet_wrap(.~basis, scales="free_y", labeller=as_labeller(basis_labeller)) +
    theme_custom() + theme(legend.position="bottom")
dev.off()
...
sql <- "SELECT SUM(CASE WHEN class=(%3$s) THEN csgd_MCF ELSE 0 END) + SUM(sopf_MCF) AS gas_flared,
        ...
        INNER JOIN %1$s ON %2$s.leaseID=%1$s.leaseID 
            AND %1$s.RunID=%2$s.RunID AND %1$s.model=%2$s.model 
...
flare_area <- dbGetQuery(db,
    sprintf(sql, "lease_info", "lease_states",
...
flare_area[, "model":= factor(model,
                        intersect(unique(model), names(model_colors)))]

svglite("./graphics/area_complete.svg", height=7, width=12, bg="transparent")
ggplot(flare_area[,
        .(gas_flared / c(1, oil_output),
            c("Volume [MCF]", "Intensity [MCF/BBL]")),
...
ggplot(flare_area) +
    geom_boxplot(aes(y=gas_flared / oil_output, x=area, fill=as.factor(time)),
        position="dodge") +
    scale_fill_manual(values=c("white", "white"), guide=NULL) +
    facet_grid(.~model) +
    labs(title="Regional differences in flaring",
        y="Flaring Intensity (MCF/BBL)", x="") +
...
ggplot(flare_area) +
    geom_boxplot(aes(y=gas_flared, x=area, fill=as.factor(time)),
        position="dodge") +
    scale_fill_manual(values=c("white", "white"), guide=NULL) +
    facet_grid(.~model) +
    labs(title="Regional differences in flaring",
        y="Flaring Volume (MCF)", x="") +
    theme_custom() + theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1))
dev.off()
...
setwd("./graphics/")
lapply(list.files(pattern=".svg$"), function(f)
    system(paste("sed -i 's/Liberation Sans/Arial/'", f)))
